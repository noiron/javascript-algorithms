...
describe('Add items to doubly linked list', function () {

    describe('Add a single item to doubly linked list', function () {
        ...
        it('First item should be hi', function () {
            ...
            expect(list._tail.data).to.be.equal('hi');
        })
        it('Second item should be null', function () {
            expect(list._head.next).to.be.null;
        })
        it('Prev pointer of head should be null', function () {
            expect(list._head.prev).to.be.null;
        })
    })

    describe('Add multiple items to doubly linked list', function () {
        var list = new doublyLinkedList();
        list.add('hello');
        list.add('world');
        list.add('good bye');
        ...
        it('First item should be hello', function () {
            ...
            expect(list._tail.prev.prev.data).to.be.equal('hello');
        })
        it('The next pointer of the second item should be good bye', function () {
        ...
        it('The tail of list is "good bye"', function () {
            expect(list._tail.data).to.be.equal('good bye');
        });

        it('The prev pointer of tail is "world"', function () {
            expect(list._tail.prev.data).to.be.equal('world');
        });

        it('Prev pointer of head should be null', function () {
            expect(list._head.prev).to.be.null;
            expect(list._tail.prev.prev.prev).to.be.null;
        });
    })
});

describe('Retrieve values from doubly linked list', function () {
    var list = new doublyLinkedList();
    list.add('hello');
    list.add('world');
    list.add('good bye');
...
describe('Remove values from linked list', function () {
    ...
    describe('Remove middle item', function () {
        ...
        it('First item should now be "hello"', function () {
            expect(list.item(0)).to.be.equal('hello');
            expect(list._tail.prev.data).to.be.equal('hello');
        });
        it('Last item should now be "good bye"', function () {
            expect(list.item(1)).to.be.equal('good bye');
        });
    ...
    describe('Remove last item', function () {
        ...
        it('Last item should now be "world"', function () {
            expect(list.item(1)).to.be.equal('world');
            expect(list._tail.data).to.be.equal('world');
        });
        it('Item in position 2 should be null', function () {
            expect(list.item(2)).to.be.null;
        });
